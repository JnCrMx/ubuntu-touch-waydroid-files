...
ListView {
    ...
    model: ListModel {
        ...
        function updateModel() {
            var current_path = folderModel.currentPath.toString()
            var path_slices = []
            ...
            for (var i = 0; i < splitted_path.length; ++i) {
                ...
                if (objPath == folderModel.currentPath.toString()/*.replace(folderModel.model.homePath(), home_prefix)*/) {
                    highlight_index = i + 1
                }
            }
    ...
    Connections {
        target: folderModel
        onCurrentPathChanged: internal.updateModel()
    }
    ...
    MouseArea {
        ...
        enabled: rootItem.contentX != 0 // && QuickUtils.mouseAttached
    ...
    Component {
        ...
        Item {
            ...
            Row {
                ...
                Label {
                    anchors.verticalCenter: parent ? parent.verticalCenter : undefined
                    width: Math.min(implicitWidth, labelMaximumWidth)
